...
async function loadLatestChart(userId) {
  try {
    const result = await query(
      `SELECT chart_data FROM kundlis WHERE user_id = $1 ORDER BY created_at DESC LIMIT 1`,
      [userId]
    );
    return result.rows[0] ? result.rows[0].chart_data : null;
  } catch (err) {
...
async function saveTurn(userId, userMessage, assistantReply) {
  try {
    await query(
...
module.exports = async (req, res) => {
  ...
  if (token && process.env.AUTH_SECRET) {
    ...
    if (payload && payload.userId) {
      ...
      chart = await loadLatestChart(userId);
    }
  }
  ...
  if (userId) await saveTurn(userId, message.trim(), reply);

  return res.status(200).json({ reply, grounded: !!chart });
};
