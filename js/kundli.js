...
(function(){
  ...
  // If we arrived here from "My Kundlis" (kundli.html?id=<id>), skip the form
  // entirely and load that specific saved chart instead of generating a new one.
  (async function loadFromHistoryIfRequested(){
    const id = new URLSearchParams(location.search).get('id');
    if(!id) return;

    const token = localStorage.getItem('vedicastro_token');
    if(!token){
      showFormError('Sign in to view your saved charts.');
      return;
    }
    ...
    try {
      const resp = await fetch(`/api/kundli?action=get&id=${encodeURIComponent(id)}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      const apiData = await resp.json().catch(() => ({}));
      if(!resp.ok) throw new Error(apiData.error || 'Could not load that chart.');
      ...
      loading.classList.remove('is-active');
      showFormError(err.message === 'Failed to fetch' ? "Couldn't reach the server." : err.message);
    }
  })();
})();
